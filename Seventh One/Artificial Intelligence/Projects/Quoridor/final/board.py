...
class Square:
    def __init__(self, x, y):
        ...
        self.rect = pygame.Rect(110 + (int(y / 2) * (squareSize + margin)),
                                110 + (int((x / 2)) * (squareSize + margin)), squareSize, squareSize)
        self.center = (
            int((y * (squareSize + margin)) + squareSize / 2), int(((x * (squareSize + margin)) + squareSize / 2)))
    ...
    def __init__(self, x, y):
        ...
        if x % 2 == 0 and y % 2 != 0:
            self.rect = pygame.Rect(
                (110 + (int(y / 2 + 1) * squareSize + int(y / 2) * margin),
                 110 + (int(x / 2) * squareSize + int(x / 2) * margin), margin,
                 squareSize))
        elif x % 2 != 0 and y % 2 == 0:
            self.rect = pygame.Rect(110 + (int(y / 2) * margin + int(y / 2) * squareSize),
                                    110 + (int(x / 2 + 1) * squareSize + int(x / 2) * margin), squareSize, margin)
        elif x % 2 != 0 and y % 2 != 0:
